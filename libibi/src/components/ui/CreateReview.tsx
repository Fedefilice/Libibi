  ...
  return (
    <form onSubmit={handleSubmit} className="create-review-form">
      ...
      <div>
        ...
        <textarea value={text} onChange={(e) => setText(e.target.value)} className="form-input h-28" />
      </div>
      {error && <div className="mt-2 p-2 bg-red-50 text-red-700 rounded">{error}</div>}
      <div className="pt-2">
        {alreadyReviewed ? (
          <div className="p-3 bg-yellow-50 text-yellow-800 rounded">Hai già scritto una recensione per questo libro.</div>
        ) : (
